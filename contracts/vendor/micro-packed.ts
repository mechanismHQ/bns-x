import * as base from "./scure-base.ts";
...
type NullableKey<T, K extends keyof T> = T[K] extends NonNullable<T[K]>
  ? never
  : K;
...
export class Reader {
  ...
  find(needle: Bytes, pos = this.pos) {
    ...
    for (
      let idx = pos;
      (idx = this.data.indexOf(needle[0], idx)) !== -1;
      idx++
    ) {
      if (idx === -1) return;
  ...
  finish() {
    ...
    throw this.err(
      `${this.leftBytes} bytes ${
        this.bitPos
      } bits left after unpack: ${base.hex.encode(this.data.slice(this.pos))}`
    );
  }
...
): BytesCoderStream<T> & BytesCoder<T> {
  return {
    ...inner,
    ...
    encode: (from: [string, T][]): Record<string, T> => {
      ...
      for (const [name, value] of from) {
        if (to[name] !== undefined)
          throw new Error(
            `coders.dict: same key(${name}) appears twice in struct`
          );
        to[name] = value;
      }
      return to;
...
const number: base.Coder<bigint, number> = {
  encode: (from: bigint): number => {
    if (from > BigInt(Number.MAX_SAFE_INTEGER))
      throw new Error(
        `coders.number: element bigger than MAX_SAFE_INTEGER=${from}`
      );
    return Number(from);
  },
...
function decimal(precision: number) {
  ...
  return {
    ...
    decode: (to: string): bigint => {
      ...
      const frac =
        BigInt(fracS.slice(0, fracLen)) * 10n ** BigInt(precision - fracLen);
...
export const string = (len: Length, le = false): CoderType<string> => {
  ...
  return wrap({
    ...
    encodeStream: (w: Writer, value: string) =>
      inner.encodeStream(w, base.utf8.decode(value)),
    decodeStream: (r: Reader): string =>
      base.utf8.encode(inner.decodeStream(r)),
  });
};
...
export const bytesFormatted = (len: Length, fmt: baseFmt, le = false) => {
  ...
  return wrap({
    size: inner.size,
    encodeStream: (w: Writer, value: string) =>
      inner.encodeStream(w, base.bytes(fmt, value)),
...
export function optional<T>(
  ...
  if (!isCoder(flag) || !isCoder(inner))
    throw new Error(
      `optional: invalid flag or inner value flag=${flag} inner=${inner}`
    );
  return wrap({
    size:
      def !== undefined && flag.size && inner.size
...
export function magic<T>(
  ...
  return wrap({
    size: inner.size,
    encodeStream: (w: Writer, value: undefined) =>
      inner.encodeStream(w, constant),
...
export const magicBytes = (constant: Bytes | string): CoderType<undefined> => {
  const c =
    typeof constant === "string" ? base.utf8.decode(constant) : constant;
...
export function map<T>(
  ...
  return wrap({
    ...
    decodeStream: (r: Reader): string => {
      ...
      if (name === undefined)
        throw r.err(
          `Enum: unknown value: ${variant} ${Array.from(variantNames.keys())}`
        );
      return name;
    },
  });
...
export function padRight<T>(
  ...
  return wrap({
    size: inner.size
      ? inner.size + padLength(blockSize, inner.size)
      : undefined,
    encodeStream: (w: Writer, value: T) => {
      const pos = w.pos;
      inner.encodeStream(w, value);
  ...
  return {
    ...
    decode(s: string): T {
      ...
      if (checksum && lines[lines.length - 1].startsWith("=")) {
        ...
        if (realCS !== cs)
          throw new Error(
            `Base64Armor: invalid checksum ${cs} instead of ${realCS}`
          );
...
export function debug<T>(inner: CoderType<T>): CoderType<T> {
  ...
  return wrap({
    size: inner.size,
    encodeStream: (w: Writer, value: T) =>
      inner.encodeStream(w, log("encode", w, value)),
