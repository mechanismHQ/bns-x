...
type AsChain<C extends Chain, Rest = Tail<C>> = {
  ...
  [K in keyof C]: Coder<
    Input<C[K]>,
    Input<K extends keyof Rest ? Rest[K] : any>
  >;
};
...
function chain<T extends Chain & AsChain<T>>(
  ...
  const encode = Array.from(args)
    .reverse()
    .reduce(
      (acc, i: any) => (acc ? wrap(acc, i.encode) : i.encode),
      undefined
    ) as any;
...
// ---------------------
export const base16: BytesCoder = chain(
  radix2(4),
...
const coderTypeError = `Invalid encoding type. Available types: ${Object.keys(
  CODERS
).join(", ")}`;
