...
import { secp256k1 as _secp, schnorr } from "./noble-curves/secp256k1.ts";
import { sha256 } from "./noble-hashes/sha256.ts";
import { ripemd160 } from "./noble-hashes/ripemd160.ts";
import {
  hex,
  base58,
  base58check as _b58,
...
import type { Coder } from "./scure-base.ts";
import * as P from "./micro-packed.ts";
...
export function cloneDeep<T>(obj: T): T {
  ...
  // slice of nodejs Buffer doesn't copy
    ...
    return Uint8Array.from(obj) as unknown as T;
  // immutable
  else if (
    ["number", "bigint", "boolean", "string", "undefined"].includes(typeof obj)
  )
    return obj;
  // null is object
...
const PubKeyECDSA = P.validate(P.bytes(null), (pub) =>
  validatePubkey(pub, PubT.ecdsa)
);
const PubKeySchnorr = P.validate(P.bytes(32), (pub) =>
...
export function ScriptNum(
  ...
  return P.wrap({
    ...
    decodeStream: (r: P.Reader): bigint => {
      ...
      if (len > bytesLimit)
        throw new Error(
          `ScriptNum: number (${len}) bigger than limit=${bytesLimit}`
        );
      if (len === 0) return 0n;
      if (forceMinimal) {
...
export const BTCArray = <T>(t: P.CoderType<T>): P.CoderType<T[]> =>
  P.array(CompactSize, t);
...
function PSBTKeyMap<T extends PSBTKeyMap>(
  psbtEnum: T
): P.CoderType<PSBTKeyMapKeys<T>> {
    ...
    decodeStream: (r: P.Reader): PSBTKeyMapKeys<T> => {
      ...
      for (const elm of raw) {
        ...
        if (byType[elm.key.type]) {
          ...
          if (!kc && key.length) {
            throw new Error(
              `PSBT: Non-empty key for ${name} (key=${hex.encode(
                key
          ...
          if (!kc) {
            if (out[name])
              throw new Error(
                `PSBT: Same keys: ${name} (key=${key} value=${value})`
              );
            out[name] = value;
            noKey[name] = true;
            continue;
        ...
        if (noKey[name])
          throw new Error(
            `PSBT: Key type with empty key and no key=${name} val=${value}`
          );
...
const PSBTInputCoder = P.validate(PSBTKeyMap(PSBTInput), (i) => {
  ...
  if (i.partialSig)
    for (const [k, v] of i.partialSig) validatePubkey(k, PubT.ecdsa);
  if (i.bip32Derivation)
    for (const [k, v] of i.bip32Derivation) validatePubkey(k, PubT.ecdsa);
  // Locktime = unsigned little endian integer greater than or equal to 500000000 representing
  if (
    i.requiredTimeLocktime !== undefined &&
    i.requiredTimeLocktime < 500000000
  )
    throw new Error(
      `validateInput: wrong timeLocktime=${i.requiredTimeLocktime}`
    );
  ...
  if (
    ...
    throw new Error(
      `validateInput: wrong heighLocktime=${i.requiredHeightLocktime}`
    );
...
const PSBTOutputCoder = P.validate(PSBTKeyMap(PSBTOutput), (o) => {
  if (o.bip32Derivation)
    for (const [k, v] of o.bip32Derivation) validatePubkey(k, PubT.ecdsa);
  return o;
});
...
function cleanPSBTFields<T extends PSBTKeyMap>(
  version: number,
  info: T,
  lst: PSBTKeyMapKeys<T>
) {
...
function validatePSBT(tx: P.UnwrapCoder<PSBTRaw>) {
  ...
  const inputCount = !version
    ? tx.global.unsignedTx!.inputs.length
    : tx.global.inputCount!;
  if (tx.inputs.length < inputCount) throw new Error("Not enough inputs");
  const inputsLeft = tx.inputs.slice(inputCount);
  if (
    inputsLeft.length > 1 ||
    (inputsLeft.length && Object.keys(inputsLeft[0]).length)
  )
    throw new Error(`Unexpected inputs left in tx=${inputsLeft}`);
  // Same for inputs
  const outputCount = !version
    ? tx.global.unsignedTx!.outputs.length
    : tx.global.outputCount!;
      ...
      if (newKV) {
        ...
        const add = (kStr: string, k: _KV[0], v: _KV[1]) => {
          ...
          if (oldVal !== newVal)
            throw new Error(
              `keyMap(${
                key as string
              }): same key=${kStr} oldVal=${oldVal} newVal=${newVal}`
            );
        };
        ...
        for (const [k, v] of newKV) {
          ...
          if (v === undefined) {
            if (cannotChange)
              throw new Error(
                `Cannot remove signed field=${key as string}/${k}`
              );
            delete map[kStr];
          } else add(kStr, k, v);
        }
...
export const p2sh = (child: P2Ret, network = NETWORK): P2Ret => {
  ...
  if (!isBytes(cs))
    throw new Error(
      `Wrong script: ${typeof child.script}, expected Uint8Array`
    );
...
const OutWSH: Coder<OptScript, OutWSHType | undefined> = {
  ...
  decode: (to: OutWSHType): OptScript =>
    to.type === "wsh" ? [0, to.hash] : undefined,
};
...
const OutWPKH: Coder<OptScript, OutWPKHType | undefined> = {
  ...
  decode: (to: OutWPKHType): OptScript =>
    to.type === "wpkh" ? [0, to.hash] : undefined,
};
...
const OutTR: Coder<OptScript, OutTRType | undefined> = {
  ...
  decode: (to: OutTRType): OptScript =>
    to.type === "tr" ? [1, to.pubkey] : undefined,
};
...
export function taprootListToTree(
  taprootList: TaprootScriptList
): TaprootScriptTree {
...
function taprootHashTree(
  ...
  if (!Array.isArray(tree)) {
    ...
    if (
      tree.tapLeafScript ||
      (tree.tapMerkleRoot && !P.equalBytes(tree.tapMerkleRoot, P.EMPTY))
    )
    ...
    const script =
      typeof leafScript === "string" ? hex.decode(leafScript) : leafScript;
    if (!isBytes(script))
  ...
  if (tree.length !== 2)
    tree = taprootListToTree(tree as TaprootNode[]) as TaprootNode[];
...
export function p2tr(
  ...
  let hashedTree = tree
    ? taprootAddPath(taprootHashTree(tree, allowUnknowOutput))
    : undefined;
  const tapMerkleRoot = hashedTree ? hashedTree.hash : undefined;
  const [tweakedPubkey, parity] = taprootTweakPubkey(
    pubKey,
    tapMerkleRoot || P.EMPTY
  );
  let leaves;
  if (hashedTree) {
...
export const p2tr_pk = (pubkey: Bytes): P2Ret =>
...
export const OutScript = P.validate(_OutScript, (i) => {
  ...
  if (
    i.type === "tr" &&
    (!isBytes(i.pubkey) || !isValidPubkey(i.pubkey, PubT.schnorr))
  )
...
export function programToWitness(
  version: number,
  data: Bytes,
  network = NETWORK
) {
...
export const _sortPubkeys = (pubkeys: Bytes[]) =>
  Array.from(pubkeys).sort(_cmpBytes);
...
export class Transaction {
  ...
  static fromPSBT(psbt: Bytes, opts: TxOpts = {}) {
    ...
    const version =
      PSBTVersion === 0 ? unsigned?.version : parsed.global.txVersion;
    const lockTime =
      PSBTVersion === 0 ? unsigned?.lockTime : parsed.global.fallbackLocktime;
    const tx = new Transaction({ ...opts, version, lockTime, PSBTVersion });
    // We need slice here, because otherwise
    const inputCount =
      PSBTVersion === 0 ? unsigned?.inputs.length : parsed.global.inputCount;
    ...
    const outputCount =
      PSBTVersion === 0 ? unsigned?.outputs.length : parsed.global.outputCount;
  ...
  toPSBT(PSBTVersion = this.opts.PSBTVersion) {
    ...
    const inputs = this.inputs.map((i) =>
      cleanPSBTFields(PSBTVersion, PSBTInput, i)
    );
    ...
    const outputs = this.outputs.map((i) =>
      cleanPSBTFields(PSBTVersion, PSBTOutput, i)
    );
    ...
    if (PSBTVersion === 0) {
      ...
      if (
        ...
        global.fallbackLocktime === DEFAULT_LOCKTIME
      )
        delete global.fallbackLocktime;
    }
  ...
  private inputSighash(idx: number) {
    ...
    const sigOutputs =
      sighash === SignatureHash.DEFAULT ? SignatureHash.ALL : sighash & 0b11;
  ...
  private normalizeInput(
    ...
    let res: PSBTKeyMapKeys<typeof PSBTInput> = {
      ...cur,
      ...i,
      nonWitnessUtxo,
      txid,
  ...
  updateInput(
    ...
    this.inputs[idx] = this.normalizeInput(
      input,
      this.inputs[idx],
      allowedFields
    );
  ...
  private normalizeOutput(
    ...
    let res: PSBTKeyMapKeys<typeof PSBTOutput> = {
      ...cur,
      ...o,
      amount,
      script,
  ...
  updateOutput(
    idx: number,
    output: TransactionOutputUpdate,
    _ignoreSignStatus = false
  ) {
    this.checkOutputIdx(idx);
    let allowedFields = undefined;
    ...
    this.outputs[idx] = this.normalizeOutput(
      output,
      this.outputs[idx],
      allowedFields
    );
  ...
  private preimageWitnessV1(
    ...
    if (
      !Array.isArray(prevOutScript) ||
      this.inputs.length !== prevOutScript.length
    )
    ...
    const outType =
      hashType === SignatureHash.DEFAULT ? SignatureHash.ALL : hashType & 0b11;
    ...
    if (outType === SignatureHash.SINGLE)
      out.push(
        idx < outputs.length ? sha256(RawOutput.encode(outputs[idx])) : EMPTY32
      );
    if (leafScript)
      out.push(
        tapLeafHash(leafScript, leafVer),
        P.U8.encode(0),
        P.I32LE.encode(codeSeparator)
      );
  ...
  signIdx(
    ...
    if (!isBytes(privateKey)) {
      ...
      if (!signers.length)
        throw new Error(
          `bip32Derivation: no items with fingerprint=${privateKey.fingerprint}`
        );
      let signed = false;
    ...
    if (inputType.txType === "taproot") {
      ...
      if (input.tapInternalKey) {
        ...
        const [taprootPubKey, parity] = taprootTweakPubkey(
          input.tapInternalKey,
          merkleRoot
        );
        if (P.equalBytes(taprootPubKey, pubKey)) {
          const hash = this.preimageWitnessV1(
            idx,
            prevOutScript,
            sighash,
            amount
      ...
      if (input.tapLeafScript) {
        ...
        for (const [cb, _script] of input.tapLeafScript) {
          ...
          const pos = scriptDecoded.findIndex(
            (i) => isBytes(i) && P.equalBytes(i, pubKey)
          );
      ...
      if (!hasPubkey)
        throw new Error(
          `Input script doesn't have pubKey: ${inputType.lastScript}`
        );
      let hash;
      if (inputType.txType === "legacy") {
        ...
        throw new Error(
          `Transaction/sign: unknown tx type: ${inputType.txType}`
        );
  ...
  finalizeIdx(idx: number) {
    ...
    if (inputType.txType === "taproot") {
      if (input.tapKeySig) input.finalScriptWitness = [input.tapKeySig];
        ...
        const leafs = input.tapLeafScript.sort(
          (a, b) =>
            TaprootControlBlock.encode(a[0]).length -
        ...
        for (const [cb, _script] of leafs) {
          ...
          const scriptSig = input.tapScriptSig.filter((i) =>
            P.equalBytes(i[0].leafHash, hash)
          );
          ...
          if (outScript.type === "tr_ms") {
            ...
            outScript.type === "unknown" &&
            this.opts.allowUnknowInput
          ) {
    ...
    let finalScriptSig: Bytes | undefined,
    ...
    if (inputType.type.startsWith("sh-wsh-")) {
      finalScriptSig = Script.encode([
        Script.encode([0, sha256(inputType.lastScript)]),
      ]);
    } else if (inputType.type.startsWith("sh-")) {
      finalScriptSig = Script.encode([
        ...Script.decode(inputScript),
  ...
  combine(other: Transaction): this {
    ...
    const thisUnsigned = this.global.unsignedTx
      ? RawTx.encode(this.global.unsignedTx)
      : P.EMPTY;
    const otherUnsigned = other.global.unsignedTx
      ? RawTx.encode(other.global.unsignedTx)
      : P.EMPTY;
