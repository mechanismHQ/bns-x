...
class BnsCard extends HTMLElement {
  ...
  connectedCallback() {
    ...
    let baseSize = 10;
    // get % width of nameEl compared to container
    const nameWidth = nameEl.clientWidth / cw;
    console.log(nameEl.clientWidth / cw);
    while (nameEl.clientWidth / cw > 0.7 && baseSize > 4) {
      console.log(cw - nameEl.clientWidth);
      console.log(nameEl.clientWidth, baseSize.toFixed(1), (cw - nameEl.clientWidth) / cw);
      baseSize -= 0.1;
